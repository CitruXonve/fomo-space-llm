...
Redis key schema
----------------
  knowledge:manifest:global
  knowledge:manifest:category:{category_id}
  knowledge:manifest:session:{session_id}
...
class KnowledgeManifest(BaseModel):
    ...
    items: dict[str, KnowledgeItemRecord] = {}   # keyed by filename
...
class KnowledgeMetadataStore:
    ...
    def load(
        ...
        if raw:
            try:
                return KnowledgeManifest.model_validate_json(raw)
    ...
    def bootstrap(
        ...
        for file_path in sorted(directory.iterdir()):
            ...
            if suffix not in _ALLOWED_SUFFIXES:
                continue
            if file_path.name in manifest.items:
                continue
            ...
            manifest.items[file_path.name] = KnowledgeItemRecord(
            ...
            changed = True
            logger.debug(
                "Bootstrap: registered '%s' in Redis manifest '%s'", file_path.name, scope_key)
...
class KnowledgeRegistry:
    ...
    Scope → key → directory mapping
    GLOBAL   → "global"              → {KB_DIRECTORY}/
    CATEGORY → "category:{id}"       → {KB_DIRECTORY}/category/{id}/
    LOCAL    → "session:{id}"        → {KB_DIRECTORY}/session/{id}/
    """

    def __init__(self, global_kb: KnowledgeBaseServiceMarkdown) -> None:
    ...
    def invalidate_global(self) -> None:
        ...
        with self._lock:
            self._global = KnowledgeBaseServiceMarkdown()
...
class KnowledgeFileService:
    ...
    def bootstrap_global(self) -> None:
        """
        Auto-register any pre-existing global files that are not yet tracked
        in the Redis manifest. Called once at application startup.
        """
        scope_key = KnowledgeRegistry.scope_key(
            ContextScope.GLOBAL, None, None)
        self._metadata.bootstrap(scope_key, self._kb_root, ContextScope.GLOBAL)
        logger.info(
    ...
    def save_file(
        ...
        if filename in manifest.items:
            existing = manifest.items[filename]
        ...
        manifest.items[filename] = record
    ...
    def get_file_info(
        ...
        """Return FileInfo for *filename* in the given scope, or None if not found."""
        target_dir = self._scope_directory(scope, session_id, category_id)
        file_path = target_dir / filename
        if not file_path.exists():
            return None
        ...
        manifest = self._metadata.load(
            scope_key, scope, category_id, session_id)
        record = manifest.items.get(filename)
        if record is None:
            return None
    ...
    def cleanup_ephemeral(self, session_id: str) -> int:
        ...
        to_delete = [
            name for name, rec in manifest.items.items()
            if rec.persistence == ContextPersistence.EPHEMERAL
        ]

        count = 0
        for name in to_delete:
            file_path = sess_dir / name
            if file_path.exists():
                file_path.unlink()
                count += 1
